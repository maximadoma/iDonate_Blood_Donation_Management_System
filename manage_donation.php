	...
	<form action="" id="manage-donation">
		...
		<div class="form-group">
			...
			<select class="custom-select select2" name="donor_id" required>
				...
				?>
				<option value="<?php echo $row['id'] ?>" data-bgroup="<?php echo $row['blood_type'] ?>" <?php echo isset($donor_id) && $donor_id == $row['id'] ? 'selected' : '' ?>><?php echo $row['name'] ?></option>
				<?php endwhile; ?>
			</select>
		</div>
		<div class="form-group">
	        <label for="" class="control-label">Blood Type</label>
			<input type="text" class="form-control" name="blood_type"  value="<?php echo isset($blood_type) ? $blood_type :'' ?>" required readonly>
			
		</div>
		<div class="form-group">
			<label for="" class="control-label">Quantity (No. of Bags)</label>
			<input type="number" class="form-control text-left" step="any" name="quantity"  value="<?php echo isset($quantity) ? $quantity :'' ?>" required>
		</div>
		<div class="form-group">
...
<script>
	...
	$('[name="donor_id"]').change(function(){
		var _id = $(this).val()
		if(_id > 0){
			$('[name="blood_type"]').val($(this).find('option[value="'+_id+'"]').attr('data-bgroup'))
		}else{
			$('[name="blood_type"]').val('')
		}
	})
