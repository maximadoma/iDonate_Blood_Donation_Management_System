...
Class Action {
	...
	function get_available(){
		...
		$inn = $this->db->query("SELECT sum(quantity) as total from blood_inventory where blood_type = '$blood_type' and status = 1 ");
		$inn = $inn->num_rows > 0 ? $inn->fetch_array()['total'] : 0;
		$out = $this->db->query("SELECT sum(quantity) as total from blood_inventory where blood_type = '$blood_type' and status = 2 $where ");
	...
	function chk_request(){
		...
		$data['data']['quantityQ'] = $data['data']['quantity']; 	
